...
function MATCHPROCESS(X, VAL) {
	let FP = 1;
	var MAIN = X;
	if(!Grid.has(X)) MAIN = Pairs.get(X), FP = -1;
	let A = 0,
		B = 0,
		GRID = Grid.get(MAIN);
	for(let i = 0; i < 3; i++)
	...
	if(FP == -1 && B >= A) return;
	if(FP == 1 && A > B) return;
	if(GRID[Math.floor((VAL - 10) / 3)][(VAL - 10) % 3]) return;
	...
	if(GRID[0][0] + GRID[1][1] + GRID[2][2] == 3) WINNER(MAIN, 0);
	else if(GRID[0][0] + GRID[1][1] + GRID[2][2] == -3) WINNER(Pairs.get(MAIN), 0);
	else if(GRID[2][0] + GRID[1][1] + GRID[0][2] == 3) WINNER(MAIN, 0);
	else if(GRID[2][0] + GRID[1][1] + GRID[0][2] == -3) WINNER(Pairs.get(MAIN), 0);
	let DRAW = 1;
	for(let i = 0; i < 3; i++) {
		for(let l = 0; l < 3; l++) {
			if(!GRID[i][l]) DRAW = 0;
